...
You must respond with a JSON object containing one fields: 'classification'.
- For 'classification', response with 0 for non-security and 1 for security, and no other classification values are allowed.
...
def process_df(df: DataFrame, type: str, llm_chain: Runnable):
	...
	for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing rows"):
		...
		tqdm.write(f"{index} {response.classification} {time_taken}")
